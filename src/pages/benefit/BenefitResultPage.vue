  ...
  <main class="result-page">
    ...
    <section class="search-summary">
      <p class="summary-text">
        <template v-if="keyword"><strong>“{{ keyword }}”</strong> 검색 결과</template>
...
const { filter, activeFilters, apiParams, queryParams, apply, clear } = useBenefitFilter(route);
...
.result-header { display: flex; align-items: flex-end; justify-content: space-between; gap: 16px; margin-bottom: 24px; }
.result-eyebrow { margin: 0 0 5px; color: #b6964d; font-size: 12px; font-weight: 700; }
.result-header h1 { margin: 0; font-size: 27px; font-weight: 800; line-height: 1.25; letter-spacing: -0.04em; }
.search-summary { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 14px; padding: 16px 18px; border-radius: 14px; background: #f8f7f2; }
.summary-text { margin: 0; color: #696158; font-size: 13px; }
.summary-text strong { color: #2e2a24; }
.result-count { flex-shrink: 0; font-size: 16px; }
.benefit-list { display: flex; flex-direction: column; gap: 12px; }
.state-card { display: flex; min-height: 220px; flex-direction: column; align-items: center; justify-content: center; padding: 28px; border: 1px solid #efece4; border-radius: 14px; text-align: center; }
.state-card p { margin: 12px 0 0; color: #908980; font-size: 13px; }
.empty-title { color: #2e2a24 !important; font-size: 16px !important; font-weight: 700; }
.loading-spinner { width: 28px; height: 28px; border: 3px solid #efece4; border-top-color: #ffbc00; border-radius: 50%; animation: spin 0.8s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
@media (max-width: 360px) { .result-page { padding-right: 16px; padding-left: 16px; } .search-summary { align-items: flex-start; flex-direction: column; } }
</style>
