<template>
  <div>
          ...
          <div>
            ...
            <div class="btn-group btn-group-sm">
              <button
                ...
                class="btn"
                :class="
                  filters.resultStatus === opt.value
                    ? 'btn-dark'
    ...
    >
      위 통계는 선택한 기간 전체 기준입니다. 결과·실행 방식 필터를 걸어도 바뀌지
      않습니다.
    </p>
              ...
              <tbody>
                ...
                >
                  ...
                  <td>
                    <span
                      class="badge"
                      :class="statusBadge(log.resultStatus)"
                    >
    ...
    >
      <div class="modal-box">
        <div class="d-flex justify-content-between align-items-start mb-3">
          <div>
            ...
            <small class="text-muted">
              ...
              <template v-if="periodText(detailTarget)">
                · {{ periodText(detailTarget) }}</template
              >
            </small>
          </div>
          <button
        ...
        <div
          v-if="detailTarget.errorMsg"
          class="error-box mb-3"
        >
          <div
            class="fw-semibold mb-1"
            :class="
              detailTarget.resultStatus === 'F'
                ? 'text-danger'
                : 'text-warning-emphasis'
            "
          >
          ...
          <template v-if="errorOf(detailTarget.errorMsg).detail">
            ...
            <pre
              v-if="showRaw"
              class="error-raw"
              >{{ errorOf(detailTarget.errorMsg).detail }}</pre
            >
          </template>
        </div>
        ...
        <template v-else>
          <div class="d-flex gap-3 mb-3 small">
            <span
              >전체 <strong>{{ details.length }}</strong
              >건</span
            >
            <span class="text-success"
              >신규 <strong>{{ insertCount }}</strong></span
            >
...
const insertCount = computed(
  () => details.value.filter((d) => d.actionType === "I").length,
);
...
async function load(page = 1) {
  ...
  try {
    ...
    loadError.value =
      "실행 기록을 불러오지 못했습니다. 서버 상태를 확인해 주세요.";
    console.error(e);
  } finally {
    loading.value = false;
  }
...
function goPage(page) {
  if (page < 1 || page > data.value.totalPages || page === data.value.page)
    return;
  load(page);
}
