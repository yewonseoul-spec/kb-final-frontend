...
const RULES = [
  ...
  },
  {
    ...
    build: (m) =>
      `DB에 없는 항목을 사용했습니다. 스키마가 최신인지 확인하세요 (${m[1]})`,
  },
  {
    test: /Table '([^']+)' doesn't exist/i,
    build: (m) =>
      `DB에 없는 테이블을 사용했습니다. 스키마가 최신인지 확인하세요 (${m[1]})`,
  },
  {
  ...
  },
  {
    ...
    build: () =>
      "다른 작업과 겹쳐 DB 처리가 지연되었습니다. 다시 시도해 주세요",
  },
  ...
  },
  {
    ...
    build: () =>
      "온통청년 서버가 응답하지 않습니다. 잠시 후 다시 시도해 주세요",
  },
  {
...
// 자동동기화 오류내용 정제
function buildRetrySummary(text) {
  const attempts = [];

  const regex =
    /\[(\d+)차(?: 전체)? 동기화 (실패|성공)\]([\s\S]*?)(?=\[\d+차(?: 전체)? 동기화 (?:실패|성공)\]|$)/g;

  let match;

  while ((match = regex.exec(text)) !== null) {
    const attemptNo = match[1];
    const result = match[2];
    const detail = match[3] || "";

    // 성공
    if (result === "성공") {
      attempts.push(`[${attemptNo}차 동기화 성공]`);

      continue;
    }

    // 실패
    const statusMatch = detail.match(/status\s*=\s*(\d{3})/i);

    if (statusMatch) {
      attempts.push(`[${attemptNo}차 동기화 실패] status=${statusMatch[1]}`);
    } else {
      attempts.push(`[${attemptNo}차 동기화 실패]`);
    }
  }
...
export function translateSyncError(raw) {
  ...
  //
  // 예:
  // [1차 동기화 실패] 전체시도=1, pageNum=8,
  // API 호출 실패: ... status=400 BAD_REQUEST
  // [2차 동기화 성공]
  //
  // 화면:
  // [1차 동기화 실패] status=400, [2차 동기화 성공]
  // ─────────────────────────────────────────────
  const retrySummary = buildRetrySummary(text);
...
/** 표 한 칸에 들어갈 만큼 자른다. */
export function shortenError(raw, limit = 80) {
